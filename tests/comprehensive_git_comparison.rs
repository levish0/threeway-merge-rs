...
fn git_merge_file(
    ...
    match algorithm {
        ...
        DiffAlgorithm::Minimal => {
            cmd.arg("--diff-algorithm").arg("minimal");
        }
        DiffAlgorithm::Patience => {
            cmd.arg("--diff-algorithm").arg("patience");
        }
        DiffAlgorithm::Histogram => {
            cmd.arg("--diff-algorithm").arg("histogram");
        }
    }
                                ...
                                if our_normalized == git_normalized && conflict_match {
                                    ...
                                    if failing_tests.len() <= 3 {
                                        ...
                                        println!(
                                            ...
                                            our.conflicts, git.conflicts
                                        );
                                        let our_preview = our_normalized
                                            .lines()
                                            .take(3)
                                            .collect::<Vec<_>>()
                                            .join("\\n");
                                        let git_preview = git_normalized
                                            .lines()
                                            .take(3)
                                            .collect::<Vec<_>>()
                                            .join("\\n");
